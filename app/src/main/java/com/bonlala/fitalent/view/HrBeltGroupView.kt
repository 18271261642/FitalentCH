...
class HrBeltGroupView : View {
    ...
    private var currentRectF : RectF ?= null
...
//        currentPaint?.let { canvas?.drawRect(RectF(0f, 0f, currentWidth, mHeight), it) }
        currentRectF = RectF(0f, 0f, currentWidth, mHeight)
//        currentRectF?.right = currentWidth
//        currentPath?.addRect(currentRectF, Path.Direction.CW)
//        currentPath?.let { currentPaint?.let { it1 -> canvas?.drawPath(it, it1) } }

        currentRectF?.let { currentPaint?.let { it1 -> canvas?.drawRect(it, it1) } }
      //  canvas?.restore()
    }
