 ...
 * @author Admin
 */
public class CircleLongClickView extends View {


    public int DEFAULT_MAX_SECONDS = 3;
    ...
    public CircleLongClickView(Context context) {
        this(context,null);
    }

    public CircleLongClickView(Context context, @Nullable AttributeSet attrs) {
        this(context, attrs ,0);
    }

    public CircleLongClickView(Context context, @Nullable AttributeSet attrs, int defStyleAttr) {
    ...
    private void getAttrs(Context context, @Nullable AttributeSet attrs) {
        TypedArray typedArray = context.obtainStyledAttributes(attrs, R.styleable.CircleLongClickView);
        //maxSeconds 最大的秒数
        mMaxSeconds = typedArray.getInt(R.styleable.CircleLongClickView_maxSeconds, DEFAULT_MAX_SECONDS);
        //annulusWidth 圆环的宽度
        mAnnulusWidth = typedArray.getInt(R.styleable.CircleLongClickView_annulusWidth, DEFAULT_ANNULUS_WIDTH);
        //annulusColor 圆环的颜色
        DEFAULT_ANNULUS_COLOR = context.getResources().getColor(com.bonlala.base.R.color.gray);
        mAnnulusColor = typedArray.getColor(R.styleable.CircleLongClickView_annulusColor, DEFAULT_ANNULUS_COLOR);
        //delayMilliseconds 进度条隔多少时间走一次,值越小走的越快，显得更流畅
        mDelayMilliseconds = typedArray.getInt(R.styleable.CircleLongClickView_delayMilliseconds, DEFAULT_RATE);
    ...
    protected void onDraw(Canvas canvas) {
        //最外层的填充圆
        canvas.drawCircle(mWidthSize / 2, mWidthSize / 2, mWidthSize / 2, mBigCirclePaint);
        //进度扇形
        RectF rectF = new RectF(0, 0, mWidthSize, mWidthSize);
        if (mCount.get() > 0) {
            //求出每一次定时器执行所绘制的扇形度数
        ...
        //中间一层灰色的圆
        canvas.drawCircle(mWidthSize / 2, mWidthSize / 2, mWidthSize / 2 - mAnnulusWidth, mMiddenCirclePaint);
        //最后绘制中心圆
        if (mIsFinish) {
    ...
    public boolean onTouchEvent(MotionEvent event) {
        Timber.e("---------touch="+event.getAction());
