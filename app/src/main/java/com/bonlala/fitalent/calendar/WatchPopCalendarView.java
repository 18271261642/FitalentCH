...
public class WatchPopCalendarView extends ImageView {
    ...
    public WatchPopCalendarView(Context context, AttributeSet attrs, int defStyle) {
        ...
        drawablePad = MiscUtil.dipToPx(getContext(),drawablePad);
        drawablePadTop = MiscUtil.dipToPx(getContext(),drawablePadTop);
...
//				LogUtil.log(TAG+" handleMessage----------");
    ...
    private void initCalendarView() {
        activeC = BikeUtils.getCalendar(-1);
        mRightNow = BikeUtils.getCalendar(-1);
        // prepare static vars
    ...
    public void setActiveC(long time) {
        activeC = BikeUtils.getCalendar(-100);
        ...
        for (int week = 0; week < mCells.length; week++) {
            for (int day = 0; day < mCells[week].length; day++) {
                if (tmp[week][day].whichMonth == CURRENT_MOUNT) { // 此月 开始设置cell
                    ...
                    mCells[week][day].setDateStr(mHelper.getYear() + "-" + BikeUtils.formatTwoStr(mHelper.getMonth() + 1) + "-" + BikeUtils.formatTwoStr(tmp[week][day].day));
...
//        }
        ...
        if (monthDataListen != null) {
            String strMonth = mRightNow.get(Calendar.YEAR) + "-" + BikeUtils.formatTwoStr(mRightNow.get(Calendar.MONTH) + 1);
    ...
    public void goCurrentMonth() {
        ...
        requestLayout();
        if (monthDataListen != null) {
            String strMonth = mHelper.getYear() + "-" + BikeUtils.formatTwoStr(mHelper.getMonth() + 1);
            monthDataListen.getMontData(strMonth);
        }
        ...
        for (Cell[] week : mCells) {
            for (Cell day : week) {
                ...
                if (day == mToday) {
                    ...
                    day.mCirclePaint.setColor(color);
                    //day.mPaint.setColor(getResources().getColor(R.color.bg_date_report_item));
                    day.mCirclePaint.setStrokeWidth(MiscUtil.dipToPx(context,1));
                    canvas.drawCircle(day.getBound().centerX(), day.getBound().centerY(), CELL_WIDTH / 3f, day
                            .mCirclePaint);
                }
                if (day == mToday) {
                    ...
                    day.mPaint.setColor(getResources().getColor(R.color.black));
                }
